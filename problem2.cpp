...
int main()
{
    ...
    for(int i=1; i<=m; i++)
    {
        int u, v; long long int w;// an edge (u,v) with a weight w.
        cin>>u>>v>>w;
    ...
    vector<vector<long long int>> D(n+1, vector<long long int>(n+1, INT_MAX));// D is a 2-D matrix for storing shortest distances between all (u,v) pairs
    ...
    for(int u=1; u<=n; u++)
    {
        for(int j=0; j<adjList[u].size(); j++)
        {
           ...
           long long int w=adjList[u][j].second;
